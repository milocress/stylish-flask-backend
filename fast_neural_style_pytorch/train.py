...
def train():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for content_batch, _ in train_loader:
            ...
            if ((batch_count - 1) % SAVE_MODEL_EVERY == 0) or (
                ...
                print(
                    "\tStyle Loss:\t{:.2f}".format(batch_style_loss_sum / batch_count)
                )
                print(
                    "\tTotal Loss:\t{:.2f}".format(batch_total_loss_sum / batch_count)
                )
                print("Time elapsed:\t{} seconds".format(time.time() - start_time))

                # Save Model
                checkpoint_path = (
                    SAVE_MODEL_PATH + "checkpoint_" + str(batch_count - 1) + ".pth"
                )
                torch.save(TransformerNetwork.state_dict(), checkpoint_path)
                print(
                    "Saved TransformerNetwork checkpoint file at {}".format(
                        checkpoint_path
                    )
                )
                ...
                sample_image_path = (
                    SAVE_IMAGE_PATH + "sample0_" + str(batch_count - 1) + ".png"
                )
