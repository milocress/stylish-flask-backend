...
import transformer
import utils
from stylize import stylize_folder, stylize_folder_single
...
def video_transfer(video_path, style_path):
    ...
    stylize_folder(
        style_path, FRAME_SAVE_PATH, STYLE_FRAME_SAVE_PATH, batch_size=BATCH_SIZE
    )
...
def getFrames(video_path):
    ...
    while success:
        cv2.imwrite(
            "{}{}{}{}".format(
                FRAME_SAVE_PATH + FRAME_CONTENT_FOLDER,
...
def makeVideo(frames_path, save_name, fps, height, width):
    ...
    images = [
        img
        for img in sorted(
            os.listdir(frames_path), key=lambda x: int(x[base_name_len:-filetype_len])
        )
        if img.endswith(".jpg")
    ]
